        ...
        try {
            ...
            if(!res.ok) {
                if(res.status === 503 || res.status === 429) {
                    ...
                    this.logger.debug(`Rate limited for ${url}, wait time in ms: ${waitingInMs}`);
                    if(this._checkRateLimitedQuery(url)) {
                        return new Promise(resolve => setTimeout(() => {
                            resolve(this._apiCall(url, method, blob));
                        }, waitingInMs));
                    } else {
                        this.logger.error(`Couldn't fetch ${url}, tryed to get it ${TIMES_RATE_LIMIT} times`);
                        return;
                    }
                } else {
